...
| AUTH-042 | B | gh_apps_perms.go:319 | `viewerCanReadRepo` had no user-to-server arm, so a `ghu_` token was indistinguishable from a session on handler-gated routes and borrowed the bearer's collaborator access to repositories the app was never installed on — strictly broader than the `ghs_` token of the same app | open — the arm was added to `viewerCanReadRepo`, which is not the choke point: 50 handlers across 24 files still call `canReadRepo` directly, and a zero-permission zero-installation app reads `/repos/{o}/{r}`, `/topics`, `/languages`, `/forks` and `/rules/branches/*` on a private repo where its own `ghs_` gets 404 |
...
| AUTH-045 | B | gh_org_hooks_rest.go:44 | `orgHookGate` checks only `canAdminOrg`, so the 11 handler-gated org-hook routes never reach `userToServerReachesOrg` — a `ghu_` token for an app installed nowhere `POST`s an org webhook pointing at an attacker URL (201) and lists them (200), while the same app's `ghs_` gets 404. Persistent exfiltration of every organization event. Same shape on `GET /orgs/{org}/installations` (ghu 200 / ghs 403) | open |
...
| AUTH-049 | B | gh_apps_perms.go:441 | `principalMayAccessTarget` ends in `return true`, so when the path names a repository that does not resolve the whole gate passes. Any handler that does not itself resolve the repo then acts on a caller-fabricated repo key — proven on repo webhooks (201 and a working hook) and Dependabot public keys (200 with the instance key). The org arm has the same shape: `if org := GetOrg(login); org != nil && !canAdminOrg(...)` skips the check for an org that does not exist, and `fineGrainedPATResourceAllowed` returns true on `repo == nil`. The path values distinguish named-but-absent from not-named, so this is fixable without over-blocking the routes that name no repository | open |
