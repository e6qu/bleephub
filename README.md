...
> **Upgrade note — repositories in an object store are in a new layout; run `bleephub adopt` once.** A repository is one object that names everything else, `<prefix>/<owner>/<repo>/manifest` (format 2), swapped by conditional write; every object is in a pack, and a pack is two objects, `pack-<sha>.pack` and `pack-<sha>.sidecar` (its index, its membership filter and a footer). Two earlier layouts are not read: the one before the manifest (a reference per object under `refs/`, `HEAD`, `packed-refs`, `.superseded` markers) and manifest format 1 (a pack's `.idx` and `.bfilter` as objects of their own, and objects written through the API loose under `objects/XX/`). A server started on a bucket written either way **refuses to start**, naming the first repository it cannot read, rather than guess — taking such a repository for an empty one would initialize over its branches. Before starting the new server, with the old one stopped and the same `BLEEPHUB_*` storage settings in the environment:
>
> ```sh
...
> ```
>
> For each repository (and each submodule repository under it) it reads the old layout — from a format-1 manifest, or before the manifest: live packs are those with a `.pack` and an `.idx` and no `.superseded` marker, superseded packs are entered as retired at their marker's time, references come from `refs/**`, `HEAD` and `packed-refs` with git's precedence — writes each live pack's sidecar from its `.idx` and `.bfilter`, packs every loose object into one pack, and writes the format-2 manifest on the condition that the manifest is still the one it read (or that there is none), printing what it wrote. A repository already in the new layout is refused and reported, so the command is safe to run again. It deletes nothing: once the new server is serving, `bleephub adopt -remove-old-layout` deletes the old reference objects, the markers, the separate `.idx` and `.bfilter` objects and the loose objects from repositories that have been adopted. `BLEEPHUB_GITSTORE_COMPACT_AFTER` (a count of loose objects, which no longer exist) is not read; `BLEEPHUB_GITSTORE_COMPACT_AFTER_PACKS` is the number of live packs above which a write asks for a compaction. See [docs/git-storage.md](docs/git-storage.md#upgrading-a-store-written-by-an-earlier-bleephub).

> **Upgrade note — secret scanning follows each repository's setting.** Content is scanned for secrets only when the repository's `security_and_analysis.secret_scanning` is enabled, as on GitHub, where a public repository is scanned automatically, a private one only with Secret Protection, and an administrator may disable it at any time. A new public repository starts with it enabled and a new private one with it disabled. Repositories created by an earlier bleephub were stored with the setting off — it was never consulted — so they are not scanned until it is enabled: `PATCH /repos/{owner}/{repo}` with `{"security_and_analysis": {"secret_scanning": {"status": "enabled"}}}`.
