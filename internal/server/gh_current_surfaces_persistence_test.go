...
func TestCurrentRESTSurfacesPersistAcrossRestart(t *testing.T) {
	...
	job := st1.CreateArtifactDeploymentJob(&ArtifactDeploymentJob{
		OrgID: org.ID, Cluster: "cluster-a", Status: "completed", TotalCount: 2,
	})
	st1.PutCodeQualityFinding(&CodeQualityFinding{
		Number: 1, RepoKey: repo.FullName, State: "open",
		Rule:     CodeQualityFindingRule{ID: "go/no-dead-code", Title: "Dead code", Severity: "warning", Category: "maintainability"},
		Location: CodeQualityFindingLocation{Path: "main.go", StartLine: 4},
		Message:  CodeQualityFindingMessage{Text: "Unused declaration", Markdown: "Unused declaration"},
	})
	pattern := st1.CreateSecretScanningCustomPatterns(customPatternScope("repo", repo.FullName), []secretScanningPatternCreate{{
	...
	first := &PullRequest{ID: 101, Number: 1, RepoID: repo.ID, BaseRefName: "main", HeadRefName: "feature-a", State: "OPEN"}
	second := &PullRequest{ID: 102, Number: 2, RepoID: repo.ID, BaseRefName: "feature-a", HeadRefName: "feature-b", State: "OPEN"}
	stack, err := st1.CreatePullRequestStack(repo, []*PullRequest{first, second})
	if err != nil {
