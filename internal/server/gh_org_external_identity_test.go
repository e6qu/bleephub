...
func TestOrganizationExternalIdentityAndTeamSyncJourney(t *testing.T) {
	...
	s.createOrg(t, "external-id-org")
	team := decodeJSON(t, s.post(t, "/api/v3/orgs/external-id-org/teams", defaultToken,
	...
	mapped := decodeJSON(t, s.patch(t, mappingPath, defaultToken, map[string]interface{}{
	...
	legacy := decodeJSON(t, s.get(t, "/api/v3/teams/"+strconv.Itoa(teamID)+"/team-sync/group-mappings", defaultToken))
	if len(legacy["groups"].([]interface{})) != 1 {
		t.Fatalf("legacy group mappings = %#v", legacy)
	}
	available := decodeJSON(t, s.get(t, "/api/v3/orgs/external-id-org/team-sync/groups", defaultToken))
	if len(available["groups"].([]interface{})) != 1 {
		t.Fatalf("available team-sync groups = %#v", available)
	}

	external := decodeJSON(t, s.get(t, "/api/v3/orgs/external-id-org/external-groups", defaultToken))
	...
	detail := decodeJSON(t, s.get(t, "/api/v3/orgs/external-id-org/external-group/"+strconv.Itoa(numericID), defaultToken))
	if detail["group_name"] != "Platform Engineers" || len(detail["teams"].([]interface{})) != 1 {
		t.Fatalf("external group detail = %#v", detail)
	}
	linked := decodeJSON(t, s.get(t, "/api/v3/orgs/external-id-org/teams/identity-team/external-groups", defaultToken))
	if len(linked["groups"].([]interface{})) != 1 {
		t.Fatalf("linked external groups = %#v", linked)
	}

	expectStatus(t, s.delete(t, "/api/v3/orgs/external-id-org/teams/identity-team/external-groups", defaultToken),
		http.StatusNoContent, "unlink external group")
	linked = decodeJSON(t, s.get(t, "/api/v3/orgs/external-id-org/teams/identity-team/external-groups", defaultToken))
	if len(linked["groups"].([]interface{})) != 0 {
		t.Fatalf("linked groups after unlink = %#v", linked)
	}
	relinked := decodeJSON(t, s.patch(t, "/api/v3/orgs/external-id-org/teams/identity-team/external-groups",
	...
	expectStatus(t, s.patch(t, mappingPath, defaultToken, map[string]interface{}{
