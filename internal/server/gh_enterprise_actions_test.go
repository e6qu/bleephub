...
func TestEnterpriseActionsCacheLimits(t *testing.T) {
	...
	resp = s.get(t, enterpriseAPI+"/actions/cache/storage-limit", defaultToken)
	if resp.StatusCode != http.StatusOK {
		resp.Body.Close()
	...
	resp = s.put(t, enterpriseAPI+"/actions/cache/storage-limit", defaultToken, map[string]interface{}{
...
func TestEnterpriseAndRepositoryActionsCacheUsagePolicy(t *testing.T) {
	...
	policy := decodeJSONWithStatus(t, s.get(t, enterprisePolicy, defaultToken), http.StatusOK)
	...
	policy = decodeJSONWithStatus(t, s.get(t, repoPolicy, defaultToken), http.StatusOK)
	if policy["repo_cache_size_limit_in_gb"] != float64(8) {
		t.Fatalf("inherited repository policy = %#v, want 8 GB", policy)
	}

	resp = s.patch(t, repoPolicy, defaultToken, map[string]interface{}{
