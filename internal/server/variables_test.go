...
func TestRepoVariablesCRUD(t *testing.T) {
	...
	mustStatus(t, s.get(t, base+"/DEPLOY_ENV", defaultToken), 404, "old name gone")
	v = decodeJSON(t, s.get(t, base+"/TARGET_ENV", defaultToken))
	if v["value"] != "production" {
	...
	mustStatus(t, s.patch(t, base+"/OTHER_VAR", defaultToken, map[string]interface{}{
		"name": "TARGET_ENV",
	...
	mustStatus(t, s.patch(t, base+"/TARGET_ENV", defaultToken, map[string]interface{}{
		"value": "x",
	}), 404, "patch missing")
}
...
func TestRepoVariablesBadNames422(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	repo := s.seedRepo(t, "var-names", false)
	base := "/api/v3/repos/" + repo.FullName + "/actions/variables"
	for _, name := range []string{"", "9LIVES", "WITH-DASH", "GITHUB_VAR", "github_var"} {
		resp := s.post(t, base, defaultToken, map[string]interface{}{"name": name, "value": "v"})
...
func TestRepoVariablesMissingRepo404(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	mustStatus(t, s.get(t, "/api/v3/repos/admin/no-such-repo/actions/variables", defaultToken), 404, "list")
	mustStatus(t, s.post(t, "/api/v3/repos/admin/no-such-repo/actions/variables", defaultToken,
...
func TestOrgVariablesLifecycle(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	org := s.seedTestOrg(t, "varorg-life")
	selRepo := s.seedOrgRepo(t, org, "var-sel-target", false)
	...
	v := decodeJSON(t, s.get(t, base+"/ORG_VAR_ALL", defaultToken))
	if v["visibility"] != "all" {
	...
	v = decodeJSON(t, s.get(t, base+"/ORG_VAR_SEL", defaultToken))
	...
	mustStatus(t, s.get(t, base+"/ORG_VAR_ALL/repositories", defaultToken), 409, "list repos on all")
	mustStatus(t, s.put(t, base+"/ORG_VAR_ALL/repositories", defaultToken,
		map[string]interface{}{"selected_repository_ids": []int{selRepo.ID}}), 409, "set repos on all")
	mustStatus(t, s.put(t, fmt.Sprintf("%s/ORG_VAR_ALL/repositories/%d", base, selRepo.ID), defaultToken, nil), 409, "add repo on all")

	repos := decodeJSON(t, s.get(t, base+"/ORG_VAR_SEL/repositories", defaultToken))
	if int(repos["total_count"].(float64)) != 1 {
		t.Fatalf("repositories total_count = %v, want 1", repos["total_count"])
	}

	other := s.seedOrgRepo(t, org, "var-sel-other", true)
	...
	mustStatus(t, s.put(t, base+"/ORG_VAR_SEL/repositories", defaultToken,
	...
	mustStatus(t, s.patch(t, base+"/ORG_VAR_SEL", defaultToken, map[string]interface{}{
		"visibility": "all",
...
func TestEnvVariablesMissingEnv404(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	repo := s.seedRepo(t, "envvar-missing", false)
	base := "/api/v3/repos/" + repo.FullName + "/environments/ghost/variables"
	mustStatus(t, s.get(t, base, defaultToken), 404, "list")
	mustStatus(t, s.post(t, base, defaultToken, map[string]interface{}{"name": "X", "value": "v"}), 404, "create")
}

func TestEnvVariablesCRUD(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	repo := s.seedRepo(t, "envvar-crud", false)
	...
	v := decodeJSON(t, s.get(t, base+"/ENV_VAR", defaultToken))
	if v["name"] != "ENV_VAR" || v["value"] != "e1" {
		t.Fatalf("get = %v", v)
	}

	mustStatus(t, s.patch(t, base+"/ENV_VAR", defaultToken, map[string]interface{}{
		"value": "e3",
	}), 204, "patch")
	list := decodeJSON(t, s.get(t, base, defaultToken))
	if int(list["total_count"].(float64)) != 1 {
	...
	mustStatus(t, s.delete(t, base+"/ENV_VAR", defaultToken), 204, "delete")
...
func TestRepoOrganizationVariablesList(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	org := s.seedTestOrg(t, "varorg-repovis")
	pubRepo := s.seedOrgRepo(t, org, "varvis-pub", false)
	privRepo := s.seedOrgRepo(t, org, "varvis-priv", true)
	base := "/api/v3/orgs/" + org.Login + "/actions/variables"
