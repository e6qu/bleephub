...
func TestOrgInvitationsLifecycle(t *testing.T) {
	...
	srv.createOrg(t, "people-inv-org")

	teamResp := srv.post(t, "/api/v3/orgs/people-inv-org/teams", defaultToken, map[string]interface{}{"name": "inv team"})
	...
	invitee, inviteeToken := srv.newUser(t, "people-invitee")

	created := srv.post(t, "/api/v3/orgs/people-inv-org/invitations", defaultToken, map[string]interface{}{
	...
	pendingMembership := srv.get(t, "/api/v3/orgs/people-inv-org/memberships/people-invitee", defaultToken)
	...
	list := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv-org/invitations", defaultToken))
	if len(list) != 1 || int(list[0]["id"].(float64)) != invID {
		t.Fatalf("pending invitations = %v", list)
	}
	teamsOfInv := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv-org/invitations/"+itoa(invID)+"/teams", defaultToken))
	if len(teamsOfInv) != 1 || teamsOfInv[0]["slug"] != "inv-team" {
		t.Fatalf("invitation teams = %v", teamsOfInv)
	}
	teamInvs := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv-org/teams/inv-team/invitations", defaultToken))
	if len(teamInvs) != 1 || int(teamInvs[0]["id"].(float64)) != invID {
		t.Fatalf("team invitations = %v", teamInvs)
	}
	legacyTeamInvs := decodeJSONArray(t, srv.get(t, "/api/v3/teams/"+itoa(teamID)+"/invitations", defaultToken))
	...
	members := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv-org/teams/inv-team/members", defaultToken))
	found := false
	...
	for name, body := range map[string]map[string]interface{}{
		...
		expectStatus(t, srv.post(t, "/api/v3/orgs/people-inv-org/invitations", defaultToken, body),
			http.StatusUnprocessableEntity, "create invitation ("+name+")")
	}

	// A plain member (not an owner) cannot create invitations.
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-inv-org/invitations", inviteeToken,
...
func TestOrgInvitationsCancelAndEmail(t *testing.T) {
	...
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-inv2-org/invitations/"+itoa(invID), defaultToken),
		http.StatusNoContent, "cancel invitation")
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-inv2-org/memberships/people-emailuser", defaultToken),
		http.StatusNotFound, "membership after cancel")
	if list := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv2-org/invitations", defaultToken)); len(list) != 0 {
	...
	created2 := srv.post(t, "/api/v3/orgs/people-inv2-org/invitations", defaultToken,
	...
	admins := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv2-org/invitations?role=admin", defaultToken))
	if len(admins) != 1 || admins[0]["email"] != "ghost@example.com" {
		t.Fatalf("role=admin filter = %v", admins)
	}
	if direct := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-inv2-org/invitations?role=direct_member", defaultToken)); len(direct) != 0 {
		t.Fatalf("role=direct_member filter = %v", direct)
	}
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-inv2-org/invitations?role=bogus", defaultToken),
...
func TestOrgFailedInvitations(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-fail-org")
	stale, _ := srv.newUser(t, "people-staleinvitee")

	created := srv.post(t, "/api/v3/orgs/people-fail-org/invitations", defaultToken,
	...
	failed := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-fail-org/failed_invitations", defaultToken))
	if len(failed) != 1 {
	...
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-fail-org/memberships/people-staleinvitee", defaultToken),
...
func TestOutsideCollaborators(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-oc-org")
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-oc-org/repos", defaultToken,
	...
	external, _ := srv.newUser(t, "people-oc-ext")
	srv.store.AddRepoCollaborator("people-oc-org", "oc-repo", external.Login, "push")
	outside := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-oc-org/outside_collaborators", defaultToken))
	...
	// collaborator…
	_, memberToken := srv.newUser(t, "people-oc-member")
	srv.activateOrgMember(t, "people-oc-org", "people-oc-member", memberToken)
	teamResp := srv.post(t, "/api/v3/orgs/people-oc-org/teams", defaultToken, map[string]interface{}{"name": "oc team", "permission": "push"})
	if teamResp.StatusCode != http.StatusCreated {
		teamResp.Body.Close()
		t.Fatalf("create team: %d", teamResp.StatusCode)
	}
	teamResp.Body.Close()
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-oc-org/teams/oc-team/memberships/people-oc-member", defaultToken, nil),
		http.StatusOK, "add team member")
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-oc-org/teams/oc-team/repos/people-oc-org/oc-repo", defaultToken,
		map[string]interface{}{"permission": "push"}), http.StatusNoContent, "grant team repo")
	for _, u := range decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-oc-org/outside_collaborators", defaultToken)) {
	...
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-oc-org/outside_collaborators/people-oc-member", defaultToken, nil),
		http.StatusNoContent, "convert member")
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-oc-org/memberships/people-oc-member", defaultToken),
		http.StatusNotFound, "membership after conversion")
	if perm := srv.store.GetRepoCollaboratorPermission("people-oc-org", "oc-repo", "people-oc-member"); perm != "push" {
		t.Fatalf("converted member repo permission = %q, want push", perm)
	}
	converted := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-oc-org/outside_collaborators", defaultToken))
	...
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-oc-org/outside_collaborators/admin", defaultToken, nil),
		http.StatusForbidden, "convert owner")
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-oc-org/outside_collaborators/nobody-here", defaultToken, nil),
		http.StatusNotFound, "convert unknown user")
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-oc-org/outside_collaborators/people-oc-ext", defaultToken, nil),
		http.StatusNotFound, "convert non-member")

	// Removing an outside collaborator strips the grants; members 422.
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-oc-org/outside_collaborators/people-oc-ext", defaultToken),
		http.StatusNoContent, "remove outside collaborator")
	if perm := srv.store.GetRepoCollaboratorPermission("people-oc-org", "oc-repo", "people-oc-ext"); perm != "" {
		t.Fatalf("removed collaborator still has permission %q", perm)
	}
	_, member2Token := srv.newUser(t, "people-oc-member2")
	srv.activateOrgMember(t, "people-oc-org", "people-oc-member2", member2Token)
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-oc-org/outside_collaborators/people-oc-member2", defaultToken),
	...
	if insecure := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-oc-org/outside_collaborators?filter=2fa_insecure", defaultToken)); len(insecure) != 0 {
		t.Fatalf("2fa_insecure filter = %v", insecure)
	}
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-oc-org/outside_collaborators?filter=bogus", defaultToken),
...
func TestOrgBlocks(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-blk-org")
	target, targetToken := srv.newUser(t, "people-blk-user")
	_ = target

	if blocked := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-blk-org/blocks", defaultToken)); len(blocked) != 0 {
		t.Fatalf("initial block list = %v", blocked)
	}
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-blk-org/blocks/people-blk-user", defaultToken),
		http.StatusNotFound, "check unblocked")

	expectStatus(t, srv.put(t, "/api/v3/orgs/people-blk-org/blocks/people-blk-user", defaultToken, nil),
		http.StatusNoContent, "block user")
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-blk-org/blocks/people-blk-user", defaultToken),
		http.StatusNoContent, "check blocked")
	blocked := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-blk-org/blocks", defaultToken))
	...
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-blk-org/invitations", defaultToken,
		map[string]interface{}{"invitee_id": target.ID}), http.StatusUnprocessableEntity, "invite blocked user")

	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-blk-org/blocks/people-blk-user", defaultToken),
	...
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-blk-org/blocks/admin", defaultToken, nil),
		http.StatusUnprocessableEntity, "block self")
	_, memberToken := srv.newUser(t, "people-blk-member")
	srv.activateOrgMember(t, "people-blk-org", "people-blk-member", memberToken)
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-blk-org/blocks/people-blk-member", defaultToken, nil),
		http.StatusUnprocessableEntity, "block member")
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-blk-org/blocks/nobody-here", defaultToken, nil),
		http.StatusNotFound, "block unknown")
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-blk-org/blocks", targetToken),
...
func TestOrgInteractionLimits(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-il-org")

	unset := srv.get(t, "/api/v3/orgs/people-il-org/interaction-limits", defaultToken)
	...
	put := srv.put(t, "/api/v3/orgs/people-il-org/interaction-limits", defaultToken,
	...
	got := decodeJSON(t, srv.get(t, "/api/v3/orgs/people-il-org/interaction-limits", defaultToken))
	if got["limit"] != "collaborators_only" {
		t.Fatalf("read-back limits = %v", got)
	}

	expectStatus(t, srv.put(t, "/api/v3/orgs/people-il-org/interaction-limits", defaultToken,
	...
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-il-org/interaction-limits", defaultToken),
		http.StatusNoContent, "delete limits")
	if body := decodeJSON(t, srv.get(t, "/api/v3/orgs/people-il-org/interaction-limits", defaultToken)); len(body) != 0 {
...
func TestOrganizationRoles(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-roles-org")

	listing := decodeJSON(t, srv.get(t, "/api/v3/orgs/people-roles-org/organization-roles", defaultToken))
	...
	single := decodeJSON(t, srv.get(t, "/api/v3/orgs/people-roles-org/organization-roles/138", defaultToken))
	if single["name"] != "all_repo_read" {
		t.Fatalf("GET role 138 = %v", single)
	}
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-roles-org/organization-roles/999999", defaultToken),
		http.StatusNotFound, "unknown role")

	teamResp := srv.post(t, "/api/v3/orgs/people-roles-org/teams", defaultToken, map[string]interface{}{"name": "roles team"})
	teamResp.Body.Close()
	...
	assignmentOf := func() string {
		t.Helper()
		users := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-roles-org/organization-roles/140/users", defaultToken))
		for _, u := range users {
	...
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-roles-org/organization-roles/users/people-roles-user/140", defaultToken, nil),
		http.StatusNoContent, "assign role to user")
	if a := assignmentOf(); a != "mixed" {
		t.Fatalf("direct+team assignment = %q, want mixed", a)
	}
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-roles-org/organization-roles/users/people-roles-user/140", defaultToken),
	...
	srv.newUser(t, "people-roles-outsider")
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-roles-org/organization-roles/users/people-roles-outsider/140", defaultToken, nil),
		http.StatusUnprocessableEntity, "assign role to non-member")
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-roles-org/organization-roles/teams/roles-team/999999", defaultToken, nil),
		http.StatusNotFound, "assign unknown role")

	// Revoking all of a team's roles clears its assignment.
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-roles-org/organization-roles/teams/roles-team", defaultToken),
		http.StatusNoContent, "revoke all team roles")
	if teams := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-roles-org/organization-roles/140/teams", defaultToken)); len(teams) != 0 {
...
func TestSecurityManagers(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-sm-org")
	teamResp := srv.post(t, "/api/v3/orgs/people-sm-org/teams", defaultToken, map[string]interface{}{"name": "sm team"})
	teamResp.Body.Close()

	expectStatus(t, srv.put(t, "/api/v3/orgs/people-sm-org/security-managers/teams/sm-team", defaultToken, nil),
		http.StatusNoContent, "add security manager team")
	managers := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-sm-org/security-managers", defaultToken))
	...
	expectStatus(t, srv.delete(t, "/api/v3/orgs/people-sm-org/security-managers/teams/sm-team", defaultToken),
		http.StatusNoContent, "remove security manager team")
	if managers := decodeJSONArray(t, srv.get(t, "/api/v3/orgs/people-sm-org/security-managers", defaultToken)); len(managers) != 0 {
		t.Fatalf("security managers after remove = %v", managers)
	}
	expectStatus(t, srv.put(t, "/api/v3/orgs/people-sm-org/security-managers/teams/no-such-team", defaultToken, nil),
...
func TestOrgMemberCopilotSeat(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-cop-org")
	...
	expectStatus(t, srv.get(t, "/api/v3/orgs/people-cop-org/members/admin/copilot", memberToken),
		http.StatusForbidden, "copilot seat of another member as non-owner")

	expectStatus(t, srv.post(t, "/api/v3/orgs/people-cop-org/copilot/billing/selected_users", defaultToken,
		map[string]interface{}{"selected_usernames": []string{"people-cop-member"}}),
		http.StatusCreated, "assign copilot seat")
	seat := decodeJSON(t, srv.get(t, "/api/v3/orgs/people-cop-org/members/people-cop-member/copilot", defaultToken))
...
func TestOrgSecurityProductEnablement(t *testing.T) {
	t.Parallel()
	srv := newIsolatedServer(t)
	srv.createOrg(t, "people-sec-org")
	for _, name := range []string{"sec-repo-a", "sec-repo-b"} {
		expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/repos", defaultToken,
	...
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/dependabot_security_updates/enable_all", defaultToken, nil),
	...
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/dependabot_alerts/disable_all", defaultToken, nil),
	...
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/advanced_security/enable_all", defaultToken, nil),
		http.StatusUnprocessableEntity, "advanced security")
	// Values outside the documented enums are not routes at all.
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/bogus_product/enable_all", defaultToken, nil),
		http.StatusNotFound, "unknown product")
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/dependabot_alerts/enable_some", defaultToken, nil),
		http.StatusNotFound, "unknown enablement")

	_, memberToken := srv.newUser(t, "people-sec-member")
	srv.activateOrgMember(t, "people-sec-org", "people-sec-member", memberToken)
	expectStatus(t, srv.post(t, "/api/v3/orgs/people-sec-org/dependabot_alerts/enable_all", memberToken, nil),
