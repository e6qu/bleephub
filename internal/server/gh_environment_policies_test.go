...
func TestEnvironmentDeploymentBranchPolicies_CRUD(t *testing.T) {
	...
	resp = s.post(t, base+"/deployment-branch-policies", defaultToken, map[string]interface{}{"name": "v*", "type": "tag"})
	...
	req, _ := http.NewRequest("POST", s.baseURL+base+"/deployment-branch-policies", jsonBody(t, map[string]interface{}{"name": "release/*"}))
	...
	resp = s.get(t, base+"/deployment-branch-policies/"+policyID, defaultToken)
	got := decodeJSONWithStatus(t, resp, 200)
	if got["name"] != "release/*" {
		t.Fatalf("get policy name = %v", got["name"])
	}

	resp = s.put(t, base+"/deployment-branch-policies/"+policyID, defaultToken, map[string]interface{}{"name": "release/*/*"})
	updated := decodeJSONWithStatus(t, resp, 200)
	if updated["name"] != "release/*/*" {
		t.Fatalf("updated name = %v", updated["name"])
	}

	delResp := s.delete(t, base+"/deployment-branch-policies/"+policyID, defaultToken)
	requireStatus(t, delResp, 204)
	resp = s.get(t, base+"/deployment-branch-policies/"+policyID, defaultToken)
	requireStatus(t, resp, 404)

	// An environment without custom branch policies rejects policy creation.
	resp = s.put(t, "/api/v3/repos/admin/"+repo+"/environments/staging", defaultToken, nil)
	requireStatus(t, resp, 200)
	resp = s.post(t, "/api/v3/repos/admin/"+repo+"/environments/staging/deployment-branch-policies", defaultToken,
...
func TestEnvironmentDeploymentProtectionRules_CRUD(t *testing.T) {
	...
	resp := s.put(t, base, defaultToken, nil)
	requireStatus(t, resp, 200)

	admin := s.store.LookupUserByLogin("admin")
	app := s.store.CreateApp(admin.ID, "Deployment Gate", "custom deployment protection rule provider",
		map[string]string{"deployments": "write"}, []string{"deployment_protection_rule"})
	s.store.CreateInstallation(app.ID, "User", admin.ID, "admin", app.Permissions, app.Events)

	// The app is available for the environment (installed on the repo owner).
	resp = s.get(t, base+"/deployment_protection_rules/apps", defaultToken)
	...
	resp = s.post(t, base+"/deployment_protection_rules", defaultToken, map[string]interface{}{"integration_id": app.ID})
	...
	resp = s.post(t, base+"/deployment_protection_rules", defaultToken, map[string]interface{}{"integration_id": 999999})
	requireStatus(t, resp, 422)

	resp = s.get(t, base+"/deployment_protection_rules", defaultToken)
	list := decodeJSONWithStatus(t, resp, 200)
	if list["total_count"] != float64(1) {
		t.Fatalf("total_count = %v, want 1", list["total_count"])
	}

	resp = s.get(t, base+"/deployment_protection_rules/"+ruleID, defaultToken)
	got := decodeJSONWithStatus(t, resp, 200)
	if got["id"] != rule["id"] {
		t.Fatalf("get rule id = %v, want %v", got["id"], rule["id"])
	}

	delResp := s.delete(t, base+"/deployment_protection_rules/"+ruleID, defaultToken)
	requireStatus(t, delResp, 204)
	resp = s.get(t, base+"/deployment_protection_rules/"+ruleID, defaultToken)
	requireStatus(t, resp, 404)

	// Deleting the environment prunes its policies and rules.
	envID := envIDForTest(t, s, repo, "production")
	if rule2 := s.store.CreateEnvProtectionRule(envID, app.ID); rule2 == nil {
		t.Fatal("re-enable rule failed")
	}
	delResp = s.delete(t, base, defaultToken)
	requireStatus(t, delResp, 204)
	if rules := s.store.ListEnvProtectionRules(envID); len(rules) != 0 {
