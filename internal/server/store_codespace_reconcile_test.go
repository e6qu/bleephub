...
// TestInterruptedCodespaceProvisioningReconcilesOnLoad pins STORE-041's remaining half: reserveCodespace commits the durable record in "Provisioning" before starting the container, so a crash there orphans it; on load the store must durably reconcile it to a resumable "Shutdown" rather than strand it in "Provisioning".
func TestInterruptedCodespaceProvisioningReconcilesOnLoad(t *testing.T) {
	...
	// Simulate the crash: a "Provisioning" record with no surviving container, plus a settled codespace alongside to prove only the interrupted one is touched.
	...
	// The heal is durable: the reconciled state must be committed, not just fixed in memory.
