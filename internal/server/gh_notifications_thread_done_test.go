...
func TestNotificationThreadMarkDone(t *testing.T) {
	...
	thread := decodeJSONWithStatus(t, s.get(t, "/api/v3/notifications/threads/"+threadID, defaultToken), 200)
	...
	del := s.delete(t, "/api/v3/notifications/threads/"+threadID, defaultToken)
	del.Body.Close()
	...
	resp = s.get(t, "/api/v3/notifications?all=true", defaultToken)
	...
	del = s.delete(t, "/api/v3/notifications/threads/999999999", defaultToken)
	del.Body.Close()
	if del.StatusCode != 404 {
		t.Fatalf("unknown thread status = %d, want 404", del.StatusCode)
	}
	del = s.delete(t, "/api/v3/notifications/threads/"+threadID, "")
	del.Body.Close()
