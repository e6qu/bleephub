...
func TestGitHubAppBrowserSettingsListAndManageInstallation(t *testing.T) {
	...
	created := s.createGitHubAppViaManifest(t, "Settings Managed App", map[string]string{"contents": "read"}, []string{"push"})
	appSlug := created["slug"].(string)

	req, _ := http.NewRequest("GET", s.baseURL+"/settings/apps", nil)
	...
	inst := s.installGitHubAppViaBrowser(t, appSlug, "admin", "all")
	instID := int(inst["id"].(float64))

	req, _ = http.NewRequest("POST", fmt.Sprintf("%s/settings/installations/%d/suspend", s.baseURL, instID), nil)
	req.Header.Set("Authorization", "token "+defaultToken)
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	...
	if got := s.store.GetInstallation(instID); got == nil || got.SuspendedAt == nil {
		t.Fatalf("installation %d was not suspended through settings route", instID)
	}

	req, _ = http.NewRequest("POST", fmt.Sprintf("%s/settings/installations/%d/unsuspend", s.baseURL, instID), nil)
	req.Header.Set("Authorization", "token "+defaultToken)
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	...
	req, _ = http.NewRequest("DELETE", fmt.Sprintf("%s/settings/installations/%d", s.baseURL, instID), nil)
	req.Header.Set("Authorization", "token "+defaultToken)
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	...
	if got := s.store.GetInstallation(instID); got != nil {
...
func TestAppManifestFlowEndToEnd(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", s.baseURL+"/settings/apps/new?state=abc123", strings.NewReader(form.Encode()))
	...
	convAgain := s.post(t, "/api/v3/app-manifests/"+code+"/conversions", "", nil)
	...
	appResp := s.get(t, "/api/v3/app", jwt)
...
func TestInstallationTokenDownscoping(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	appID, _, pemKey, instID := s.installAppOnOrg(t, "downscope-org", "scoped-repo",
	...
	allowed := s.post(t, "/api/v3/repos/downscope-org/scoped-repo/issues", fullToken,
...
func TestInstallationTokenCreatesOrganizationRepositoryWithAdministrationPermission(t *testing.T) {
	...
	appData := s.createGitHubAppViaManifest(t, "Organization Repo Creator App", map[string]string{
	...
	tokenResp := s.post(t, fmt.Sprintf("/api/v3/app/installations/%d/access_tokens", instID), jwt, nil)
	...
	createResp := s.post(t, "/api/v3/orgs/"+orgLogin+"/repos", token, map[string]interface{}{
...
func TestInstallationTokenCreateOrganizationRepositoryRequiresAdministrationWrite(t *testing.T) {
	...
	appData := s.createGitHubAppViaManifest(t, "Organization Repo Metadata App", map[string]string{
		"metadata": "read",
	}, nil)
	appID := int(appData["id"].(float64))
	pemKey := appData["pem"].(string)
...
func TestInstallationSuspensionBlocksTokens(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	appID, _, pemKey, instID := s.installAppOnOrg(t, "suspend-org", "suspend-repo",
	...
	mint := s.post(t, tokenPath, jwt, nil)
	...
	after := s.get(t, "/api/v3/installation/repositories", tokenStr)
	after.Body.Close()
	...
	revReq, _ := http.NewRequest("DELETE", s.baseURL+"/api/v3/installation/token", nil)
	...
	gone := s.get(t, "/api/v3/installation/repositories", tokenStr)
	gone.Body.Close()
...
func TestOrgInstallationsList(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	appID, appSlug, _, instID := s.installAppOnOrg(t, "instlist-org", "instlist-repo",
		map[string]string{"contents": "read"})

	resp := s.get(t, "/api/v3/orgs/instlist-org/installations", defaultToken)
	if resp.StatusCode != http.StatusOK {
		resp.Body.Close()
