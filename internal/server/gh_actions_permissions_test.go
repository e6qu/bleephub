...
func TestOrgActionsPermissions_ArtifactAndLogRetention(t *testing.T) {
	...
	data := decodeJSONWithStatus(t, srv.get(t, path, defaultToken), 200)
	...
	resp := srv.put(t, path, defaultToken, map[string]int{"days": 30})
	resp.Body.Close()
	...
	resp = srv.put(t, path, defaultToken, map[string]int{"days": 500})
	resp.Body.Close()
...
func TestOrgActionsPermissions_ForkPRContributorApproval(t *testing.T) {
	...
	resp := srv.put(t, path, defaultToken, map[string]string{"approval_policy": "all_external_contributors"})
	resp.Body.Close()
	if resp.StatusCode != 204 {
		t.Fatalf("put = %d, want 204", resp.StatusCode)
	}
	...
	resp = srv.put(t, path, defaultToken, map[string]string{"approval_policy": "nobody"})
	resp.Body.Close()
	if resp.StatusCode != 422 {
...
func TestOrgActionsPermissions_ForkPRWorkflowsPrivateRepos(t *testing.T) {
	...
	resp := srv.put(t, path, defaultToken, map[string]bool{
	...
	resp = srv.put(t, path, defaultToken, map[string]bool{"send_secrets_and_variables": true})
	resp.Body.Close()
	if resp.StatusCode != 422 {
...
func TestOrgActionsPermissions_SelfHostedRunners(t *testing.T) {
	...
	resp := srv.put(t, path, defaultToken, map[string]string{"enabled_repositories": "selected"})
	resp.Body.Close()
	if resp.StatusCode != 204 {
		t.Fatalf("put = %d, want 204", resp.StatusCode)
	}
	...
	resp = srv.put(t, path, defaultToken, map[string]string{"enabled_repositories": "sometimes"})
	resp.Body.Close()
	if resp.StatusCode != 422 {
	...
	resp = srv.put(t, fmt.Sprintf("%s/repositories/%d", path, repoID), defaultToken, nil)
	resp.Body.Close()
	if resp.StatusCode != 204 {
		t.Fatalf("add repo = %d, want 204", resp.StatusCode)
	}
	resp = srv.put(t, path+"/repositories/999999", defaultToken, nil)
	resp.Body.Close()
	if resp.StatusCode != 404 {
		t.Fatalf("add unknown repo = %d, want 404", resp.StatusCode)
	}
	list := decodeJSONWithStatus(t, srv.get(t, path+"/repositories", defaultToken), 200)
	...
	resp = srv.delete(t, fmt.Sprintf("%s/repositories/%d", path, repoID), defaultToken)
	resp.Body.Close()
	if resp.StatusCode != 204 {
...
func TestOrganizationsCachePolicyLimits(t *testing.T) {
	...
	srv := newIsolatedServer(t)
	org := srv.createTestOrg(t)
	orgID := strconv.Itoa(srv.store.GetOrg(org).ID)
	...
	data := decodeJSONWithStatus(t, srv.get(t, retention, defaultToken), 200)
	if data["max_cache_retention_days"].(float64) != 90 {
		t.Fatalf("default max_cache_retention_days = %v, want 90", data)
	}
	resp := srv.put(t, retention, defaultToken, map[string]int{"max_cache_retention_days": 14})
	resp.Body.Close()
	if resp.StatusCode != 204 {
	...
	resp = srv.put(t, retention, defaultToken, map[string]int{"max_cache_retention_days": 0})
	resp.Body.Close()
	if resp.StatusCode != 400 {
		t.Fatalf("invalid retention put = %d, want 400", resp.StatusCode)
	}

	data = decodeJSONWithStatus(t, srv.get(t, storage, defaultToken), 200)
	if data["max_cache_size_gb"].(float64) != 10 {
		t.Fatalf("default max_cache_size_gb = %v, want 10", data)
	}
	resp = srv.put(t, storage, defaultToken, map[string]int{"max_cache_size_gb": 25})
	resp.Body.Close()
	if resp.StatusCode != 204 {
...
func TestOrgCacheUsage_ComputedFromRealCacheStore(t *testing.T) {
	...
	data := decodeJSONWithStatus(t, srv.get(t, usagePath, defaultToken), 200)
	...
	byRepo := decodeJSONWithStatus(t, srv.get(t, byRepoPath, defaultToken), 200)
...
func TestRunnerLabels_AddAndRemoveSingle(t *testing.T) {
	...
	srv := newIsolatedServer(t)
	org := srv.createTestOrg(t)
	repoResp := srv.post(t, "/api/v3/orgs/"+org+"/repos", defaultToken,
	...
	resp = srv.delete(t, orgPath+"/self-hosted", defaultToken)
	resp.Body.Close()
	if resp.StatusCode != 422 {
