...
func TestDeleteOrgCascadesPackagesAndMarketplace(t *testing.T) {
	...
	st.Mu.Lock()
	pkg := &store.Package{ID: 1, Name: "lib", PackageType: "npm", OwnerType: "Organization", OwnerKey: "acme"}
	st.Packages[1] = pkg
	st.PackagesByOwnerKey["acme"] = map[string]*store.Package{store.PackageKey("npm", "lib"): pkg}
	ver := &store.PackageVersion{ID: 10, PackageID: 1}
	st.PackageVersions[10] = ver
	st.PackageVersionsByPackage[1] = map[int]*store.PackageVersion{10: ver}
	file := &store.PackageFile{ID: 100, VersionID: 10, StoragePath: "objkey-1"}
	st.PackageFiles[100] = file
	st.PackageFilesByVersion[10] = map[int]*store.PackageFile{100: file}
	st.Mu.Unlock()
	bs.blobs["objkey-1"] = []byte("payload")

	// A Marketplace purchase held by the org.
	mkey := store.MarketplacePurchaseKey("plan-slug", "Organization", org.ID)
	st.Misc.Mu.Lock()
	st.Misc.MarketplacePurchases[mkey] = &store.MarketplacePurchase{ListingSlug: "plan-slug", AccountType: "Organization", AccountID: org.ID}
	st.Misc.Mu.Unlock()
...
func TestDeleteUserCascadesOwnedResources(t *testing.T) {
	...
	user := &store.User{ID: 2, Login: "bob", Type: "User"}
	st.Mu.Lock()
	st.Users[2] = user
	st.UsersByLogin["bob"] = user
	// A package owned directly by the user, with a version + file in the store.
	pkg := &store.Package{ID: 1, Name: "cli", PackageType: "npm", OwnerType: "User", OwnerKey: "bob"}
	st.Packages[1] = pkg
	st.PackagesByOwnerKey["bob"] = map[string]*store.Package{store.PackageKey("npm", "cli"): pkg}
	ver := &store.PackageVersion{ID: 10, PackageID: 1}
	st.PackageVersions[10] = ver
	st.PackageVersionsByPackage[1] = map[int]*store.PackageVersion{10: ver}
	file := &store.PackageFile{ID: 100, VersionID: 10, StoragePath: "user-objkey"}
	st.PackageFiles[100] = file
	st.PackageFilesByVersion[10] = map[int]*store.PackageFile{100: file}
	st.Memberships["9:2"] = &store.Membership{OrgID: 9, UserID: 2, State: store.MembershipStateActive}
	st.Mu.Unlock()
	bs.blobs["user-objkey"] = []byte("payload")
	mkey := store.MarketplacePurchaseKey("plan-slug", "User", 2)
	st.Misc.Mu.Lock()
	st.Misc.MarketplacePurchases[mkey] = &store.MarketplacePurchase{ListingSlug: "plan-slug", AccountType: "User", AccountID: 2}
	st.Misc.Mu.Unlock()
