...
func scheduleRunCounter(repoKey string) func() int {
	return func() int {
...
func TestFireSchedulesThroughReplaysMissedMinutes(t *testing.T) {
	...
	commitWorkflowYAMLToStorage(t, testServer, repoKey, ".github/workflows/every5.yml", `name: every5
on:
  schedule:
    - cron: '*/5 * * * *'
jobs:
  build:
    ...
    steps:
      - run: echo tick
`)
	countRuns := scheduleRunCounter(repoKey)
	...
	cursor := testServer.fireSchedulesThrough(last, now)
	...
	testServer.fireSchedulesThrough(cursor, now.Add(90*time.Second))
	if got := countRuns(); got != 2 {
...
func TestFireDueSchedulesDeduplicatesDSTFallBack(t *testing.T) {
	...
	cancelRepoRunsCleanup(t, repoKey)
	commitWorkflowYAMLToStorage(t, testServer, repoKey, ".github/workflows/nightly-tz.yml", `name: nightly-tz
on:
  schedule:
    - cron: '30 1 * * *'
...
`)
	...
	testServer.fireDueSchedules(time.Date(2026, 11, 1, 5, 30, 0, 0, time.UTC))
	if got := countRuns(); got != 1 {
		t.Fatalf("first local 01:30 (05:30 UTC) fired %d runs, want 1", got)
	}
	testServer.fireDueSchedules(time.Date(2026, 11, 1, 6, 30, 0, 0, time.UTC))
	if got := countRuns(); got != 1 {
