...
//
// Counting them diffs every file the commit changed, line by line, and the
// statistics endpoints count them for every commit of the default branch on
// every request. They are a function of the commit's id alone — the id fixes
// the commit, its parent and every byte of both trees — so what has been
