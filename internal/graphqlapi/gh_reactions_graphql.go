...
// initReactionGraphQLTypes creates the shared Reaction, ReactionConnection and
// Reactable types, so concrete subject types can declare they implement
// Reactable. The interface's ResolveType reads the registry lazily, so it may
// name concrete types not yet built.
func (s *Resolver) initReactionGraphQLTypes(userType *graphql.Object) {
	...
	// Reactable is implemented by the nine concrete subject types. ResolveType
	// discriminates on the source node-id prefix, most-specific first.
	...
	// Reaction fields that depend on the Reactable interface, added now it
	// exists. reactable resolves the reacted-to subject.
	...
	// CommitComment is the one Reactable subject with no pre-existing type.
	s.initCommitCommentType()
}

// initCommitCommentType creates the CommitComment type, the ninth Reactable
// concrete type.
...
func commitCommentToGQL(c *store.CommitComment, st *store.Store) map[string]interface{} {
	return map[string]interface{}{
		...
		// A removed author must be an untyped nil, not a nil map, or the Actor
		// type's non-null fields abort the surrounding subtree.
		"author": optionalRendered(st.GetUserByID(c.AuthorID), userToGraphQL),
	}
}

// reactionConnectionArgs is the shared Relay pagination arg set for every
// reactions: ReactionConnection! field (a subset of GitHub's args).
...
// addReactableFields attaches the Reactable fields (bound to the subject's
// parentType) to a concrete type; its config must also declare Interfaces:
// [reactable].
...
// reactableDBID extracts a subject's database id from its source map, accepting
// either "databaseId" or the internal "_dbID".
...
// reactableFields returns the Reactable field set bound to a store parentType.
// addReactableFields installs them via AddFieldConfig so every reactable type
// satisfies the interface uniformly.
func (s *Resolver) reactableFields(parentType string) graphql.Fields {
	return graphql.Fields{
		...
		"viewerCanReact": &graphql.Field{
			Type: graphql.NewNonNull(graphql.Boolean),
			// The subject was reached through a readable connection, so
			// authentication is the operative check.
			Resolve: func(p graphql.ResolveParams) (interface{}, error) {
...
// viewerReactorID returns the viewer's user ID, or 0 when anonymous.
...
// graphQLToReactionContent maps the ReactionContent enum to the store's content
// strings (inverse of reactionContentToGraphQL).
...
// databaseID) the reaction attaches to, plus the owning repo and author for
// authorization.
...
// resolving all nine concrete types. The parentType strings mirror the REST
// reaction handlers so both surfaces key the same store rows.
...
// reactableSubjectSource builds the concrete source map for a reacted subject
// so the payload's subject: Reactable field resolves to a populated type.
// Returns nil if the subject no longer exists.
...
// resolveReactableSubject decodes a Reactable node id to its store (parentType,
// databaseID). ok=false when the id is not a supported subject.
...
// reactableSubjectScope maps a reaction subject to its required app permission
// scope (issues, pull_requests or discussions — never one blanket scope).
...
// reactableScope resolves the input's subject and returns its required scope,
// for the scopeFor hook on the reaction authz rows.
...
func mutationTargetReactable(key string) func(*Resolver, map[string]interface{}) mutationTarget {
	return func(s *Resolver, input map[string]interface{}) mutationTarget {
		nodeID, _ := input[key].(string)
		// Keep `missing` set so authorize errors rather than silently
		// authorizing when the subject doesn't resolve.
...
// addReactionMutationsToSchema registers addReaction/removeReaction (the write
// side; reactionGroups already served the read side).
