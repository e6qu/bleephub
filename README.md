...
**OAuth Apps.** Distinct entity from GitHub Apps. Browser settings own OAuth App setup and its full owner lifecycle: list/create/get/update/delete plus one-time client-secret rotation; deletion revokes every derived access and refresh token. OAuth web flow (`/login/oauth/authorize`) requires the real login session plus CSRF consent POST before issuing an authorization code; device flow lives at `/login/device/code`, and the browser can select either registered OAuth Apps or GitHub Apps without copying a client identifier. Token-management family on the real `/api/v3/applications/{client_id}/{token,grant}` (check / reset / revoke / scope).
...
Every metadata transaction advances a monotonic revision. A dqlite-backed replica loads and serves only revision-stable snapshots, reloading when a peer has written while preserving explicitly process-local runner connections, messages, jobs, and log caches. If rollback after a failed write cannot reach persistence, later requests fail closed until a durable reload succeeds. Scheduled workflow minute claims use a separate durable uniqueness table, so overlapping tasks during a deployment cannot launch the same cron run twice. Git refs are serialized per repository/ref across processes; updates use compare-and-set, creates require absence, and deletes require the value the handler observed.
...
- **Response-shape ratchet** (`openapi_shape_validator_test.go`) — an observer on the shared test server validates every 2xx `/api/v3` JSON response member-by-member against the documented response schema. A response whose status the description does not document, or documents without a body, is reported (`undocumented-status` / `undocumented-body`) rather than skipped. Violations are gated against [`internal/server/openapi-violation-allowlist.txt`](internal/server/openapi-violation-allowlist.txt) — the only copy, enforced by `TestViolationAllowlistIsSingleCopy` and resolved relative to the package directory. Each block states either a checked citation into an official description or, where there is none, that the member is unverified and the emitter is what has to change; only `/meta`'s `installed_version` is currently in the first category.
  ...
  The description itself is pinned: `third_party/github-openapi.VERSION` records the `github/rest-api-description` commit and the SHA-256 of the uncompressed bytes, and `TestVendoredOpenAPIMatchesRecordedPin` fails if the vendored file is not the one recorded.
