...
func TestOrgCopilotBillingSeats_ValidationAndPermissions(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	org, members := s.copilotTestOrg(t, "copilot-validation-org", "copilot-val-member")
	base := "/api/v3/orgs/" + org.Login
	outsider := seedTestUser(s.Server, "copilot-outsider")

	// Unknown username → 422, nothing assigned.
	requireStatus(t, s.post(t, base+"/copilot/billing/selected_users", defaultToken,
	...
	memberTok := s.store.CreateToken(members[0].ID, "admin:org").Value
	requireStatus(t, s.get(t, base+"/copilot/billing", memberTok), 403)
	requireStatus(t, s.post(t, base+"/copilot/billing/selected_users", memberTok,
		map[string]interface{}{"selected_usernames": []string{members[0].Login}}), 403)

	// Unknown org → 404.
	requireStatus(t, s.get(t, "/api/v3/orgs/copilot-no-such-org/copilot/billing", defaultToken), 404)

	// Member without a seat → 404; unknown member → 404.
	requireStatus(t, s.get(t, base+"/members/"+members[0].Login+"/copilot", defaultToken), 404)
	requireStatus(t, s.get(t, base+"/members/copilot-no-such-user/copilot", defaultToken), 404)
...
func TestOrgCopilotBillingSeats_Teams(t *testing.T) {
	...
	// Unknown team → 422.
	requireStatus(t, s.post(t, base+"/copilot/billing/selected_teams", defaultToken,
		map[string]interface{}{"selected_teams": []string{"no-such-team"}}), 422)

	// Assign by team name: every member gets a seat.
	resp := s.post(t, base+"/copilot/billing/selected_teams", defaultToken,
	...
	requireStatus(t, s.do(t, "DELETE", base+"/copilot/billing/selected_users", defaultToken,
		map[string]interface{}{"selected_usernames": []string{members[0].Login}}), 422)

	// Cancelling the team cancels every seat it assigned.
	resp = s.do(t, "DELETE", base+"/copilot/billing/selected_teams", defaultToken,
...
func TestOrgCopilotMetrics_HonestlyEmptyWithoutActivity(t *testing.T) {
	...
	for _, path := range []string{base + "/copilot/metrics", base + "/team/" + team.Slug + "/copilot/metrics"} {
		...
		req, _ := newGHRequest("GET", s.baseURL+path, defaultToken)
	...
	requireStatus(t, s.get(t, base+"/team/no-such-team/copilot/metrics", defaultToken), 404)
	requireStatus(t, s.get(t, base+"/copilot/metrics?since=yesterday", defaultToken), 422)
	...
	for _, report := range []string{"organization-1-day", "users-1-day", "user-teams-1-day"} {
		...
		requireStatus(t, s.get(t, path, defaultToken), 422)
		requireStatus(t, s.get(t, path+"?day=not-a-day", defaultToken), 422)
		requireStatus(t, s.get(t, path+"?day=2026-07-01", defaultToken), 204)
	}
	...
	for _, report := range []string{"organization-28-day", "users-28-day"} {
		body := decodeJSONWithStatus(t, s.get(t, base+"/copilot/metrics/reports/"+report+"/latest", defaultToken), 200)
...
func TestOrgCopilotContentExclusion_RoundTrip(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	org, _ := s.copilotTestOrg(t, "copilot-exclusion-org")
	base := "/api/v3/orgs/" + org.Login + "/copilot/content_exclusion"

	// Unconfigured organizations have no rules.
	if rules := decodeJSONWithStatus(t, s.get(t, base, defaultToken), 200); len(rules) != 0 {
	...
	if msg := decodeJSONWithStatus(t, s.put(t, base, defaultToken, rules), 200); msg["message"] == nil {
		t.Fatalf("PUT response = %v, want a message", msg)
	}
	got := decodeJSONWithStatus(t, s.get(t, base, defaultToken), 200)
	if len(got) != 2 {
	...
	requireStatus(t, s.put(t, base, defaultToken, map[string]interface{}{
	...
	requireStatus(t, s.put(t, base, defaultToken, map[string]interface{}{"*": []interface{}{42}}), 422)
	requireStatus(t, s.put(t, base, defaultToken, map[string]interface{}{
...
func TestOrgCopilotCodingAgentPermissions_RepoScopedPolicy(t *testing.T) {
	t.Parallel()
	s := newIsolatedServer(t)
	org, _ := s.copilotTestOrg(t, "copilot-agent-org")
	admin := s.store.LookupUserByLogin("admin")
	repo1 := s.store.CreateOrgRepo(org, admin, "agent-repo-1", "", false)
	repo2 := s.store.CreateOrgRepo(org, admin, "agent-repo-2", "", false)
	foreign := s.store.CreateRepo(admin, "copilot-agent-foreign-repo", "", false)
	...
	// is "selected".
	requireStatus(t, s.get(t, base+"/repositories", defaultToken), 409)
	requireStatus(t, s.put(t, base+"/repositories/"+fmt.Sprint(repo1.ID), defaultToken, nil), 409)
	...
	requireStatus(t, s.put(t, base+"/repositories", defaultToken,
	...
	requireStatus(t, s.put(t, base+"/repositories/"+fmt.Sprint(repo2.ID), defaultToken, nil), 204)
	requireStatus(t, s.put(t, base+"/repositories/999999", defaultToken, nil), 404)
	requireStatus(t, s.put(t, base+"/repositories/"+fmt.Sprint(foreign.ID), defaultToken, nil), 422)
...
func TestRepoCopilotCloudAgentConfiguration(t *testing.T) {
	...
	s := newIsolatedServer(t)
	admin := s.store.LookupUserByLogin("admin")
	repo := s.store.CreateRepo(admin, "cloud-agent-config-repo", "", false)
	path := "/api/v3/repos/" + repo.FullName + "/copilot/cloud-agent/configuration"

	cfg := decodeJSONWithStatus(t, s.get(t, path, defaultToken), 200)
	...
	requireStatus(t, s.get(t, "/api/v3/repos/admin/no-such-repo/copilot/cloud-agent/configuration", defaultToken), 404)
	requireStatus(t, s.get(t, path, ""), 401)
}
