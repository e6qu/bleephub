...
func TestZeroOptionsSelectEveryDefault(t *testing.T) {
	...
	if got.Region != "us-east-1" || got.ChunkBytes != 4<<20 || got.CacheBytes != 8<<30 ||
		...
		got.CompactionTrigger != 4096 || got.MultipartBytes != 64<<20 ||
...
func TestNegativeOptionsSelectOff(t *testing.T) {
	got := Options{MemoryCacheBytes: -1, IndexFreshness: -1, CompactionTrigger: -1, BreakerThreshold: -1}.resolved()
	if got.MemoryCacheBytes != 0 || got.IndexFreshness != 0 || got.CompactionTrigger != 0 || got.BreakerThreshold != 0 {
...
func TestExplicitOptionsSurviveResolution(t *testing.T) {
	want := Options{
		...
		IndexFreshness: time.Hour, CompactionTrigger: 7, MultipartBytes: 1024, BreakerThreshold: 2, BreakerCooldown: time.Minute,
	}
